...
export default function JobCard({ job, onAuthRequired }) {
  ...
  const handleSaveJob = async (e) => {
    ...
    try {
      if (saved) {
        const res = await axios.delete(`${USER_API_END_POINT}/unsaveJob/${job._id}`, { withCredentials: true });
        if (res.data.success) {
          setSaved(false);
      ...
      toast.error(err.response?.data?.message || 'Failed to save/unsave job');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        {/* Job Title */}
        <h3 className="font-bold text-xl text-gray-900 mb-4 line-clamp-2 min-h-[3.5rem] leading-tight group-hover:text-purple-900 transition-colors duration-300">
          {job.title}
        </h3>
